...
function getRandomColorSequence(colors, length){
    ...
    for( let i = 0 ; i < length; i++){
        array[i] = colors[parseInt(Math.random()*colors.length)]
    }
...
function getRockPaperScissor(){
   let posibilidades = ["rock", "paper","scissor"]
   let resultado = posibilidades[(Math.random()*3)];
   return resultado;
...
function getRandomColorSequence(length){
    let array = [];
    const posibilidades = ["rock", "paper" , "scissor"]
    for( let i = 0 ; i < length; i++){
        array[i] = posibilidades[parseInt(Math.random()*colors.length)]
    }
...
function  filterNumbersGreaterThan(numbers, filter){
    ...
    for(let i = 0 ; i< numbers.length;i++){
        if(numbers[i]< filter){
            numerosFiltrados.push(numbers[i]);
        }
    }
...
console.log(toHackerSpeak("pepe"))
